...
- `scripts/analyze_crawler_noise.py` — Validate/improve crawler `remove_html_elements` selectors against the ingested node HTML (run via `uv run --with beautifulsoup4`).
...
- **Priority: Strip Boilerplate:** Set `remove_html_elements` to remove headers, menus, navs, breadcrumbs, footers, cookie banners, and embed wrappers. For Framer/Next "mco-*" sites, use the teaser/CTA selector set in `references/data-sources.md` and validate before a full crawl with `scripts/analyze_crawler_noise.py` (dumps ingested node HTML → reports clean-vs-raw ratios, residual noise marks, surviving body phrases, and the exact new `[class*="..."]` selectors to append). Iterate until clean.
